...
public final class NoSqlDatabaseManager<W> extends AbstractDatabaseManager {
    ...
    protected void writeInternal(final LogEvent event) {
        ...
        //	Customization to handle inserting JSON into "message" field
        Class<?> eventMessageClass = event.getMessage().getClass();

        if (eventMessageClass.isAssignableFrom(ObjectMessage.class))
        {
        	ObjectMessage objectMessage = (ObjectMessage) event.getMessage();

        	Object[] objectMessageArray = objectMessage.getParameters();

        	entity.set("message", objectMessageArray[0]);
        }
        else
        {
