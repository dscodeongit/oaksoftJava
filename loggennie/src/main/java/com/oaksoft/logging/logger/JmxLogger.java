...
@Plugin(name = "JmxLogger", category = Node.CATEGORY, printObject = true)
public class JmxLogger extends LoggerConfig{
    ...
    public static LoggerConfig createLogger(
    	...
    	SourceConnector.connect(sourceConfig, SourceType.JMX);
    	    	
		Level actualLevel = (level == null) ? Level.ERROR : level;
		boolean additive = Booleans.parseBoolean(additivity, true);
		return LoggerConfig.createLogger(additive, actualLevel, JmxCollector.class.getPackage().getName(), includeLocation, refs, properties, config, filter);
    }
}
